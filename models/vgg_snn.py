...
class VGG16(nn.Module):
    ...
    def features(self):
        ...
        self.lif5 = snn.Leaky(beta=self.beta)
        self.conv6 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.lif6 = snn.Leaky(beta=self.beta)
        self.conv7 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.lif7 = snn.Leaky(beta=self.beta)
        self.mp3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv8 = nn.Conv2d(256, 512, kernel_size=3, padding=1)
        self.lif8 = snn.Leaky(beta=self.beta)
        self.conv9 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.lif9 = snn.Leaky(beta=self.beta)
        self.conv10 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.lif10 = snn.Leaky(beta=self.beta)
        self.mp4 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv11 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.lif11 = snn.Leaky(beta=self.beta)
        self.conv12 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.lif12 = snn.Leaky(beta=self.beta)
        self.conv13 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.lif13 = snn.Leaky(beta=self.beta)
        self.mp5 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.lif14 = snn.Leaky(beta=self.beta)
        # self.avgpool = nn.AdaptiveAvgPool2d((2, 2))

    def classifier(self):
        self.flt1 = nn.Flatten()
        self.lin1 = nn.Linear(512 * 2 * 2, 256)
        self.lif15 = snn.Leaky(beta=self.beta)
        self.drp1 = nn.Dropout()
        self.lin2 = nn.Linear(256, 64)
        self.lif16 = snn.Leaky(beta=self.beta)
        self.drp2 = nn.Dropout()
        self.lin3 = nn.Linear(64, self.num_classes)
        self.lif17 = snn.Leaky(beta=self.beta, spike_grad=self.spike_grad, output=True)
